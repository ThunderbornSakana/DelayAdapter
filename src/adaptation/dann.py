    ...
    def learn(self, source_loader, target_loader, test_loader, save_path, device):
        ...
        for epoch in range(num_epoch):
            ...
            for i, (src_num, src_cat, _, y) in enumerate(source_loader):
                ...
                # Make classifications on source domain data.
                src_pred = torch.sigmoid(self.clf(src_feat)).squeeze()
                # Make domain classifications.
                alpha = self.get_lambda(epoch, num_epoch)
                src_feat = ReverseLayerF.apply(src_feat, alpha)
                tgt_feat = ReverseLayerF.apply(tgt_feat, alpha)
                src_d_pred = torch.sigmoid(self.discriminator(src_feat))
                tgt_d_pred = torch.sigmoid(self.discriminator(tgt_feat))
                # Compute loss.
                clf_loss = criterion(src_pred, y)
